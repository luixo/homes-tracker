...
const parsePart = (
	type: string,
	text: string,
): Partial<TrackerRequest["filter"]> | null => {
	switch (type) {
		case "price-total": {
...
export const parseFilters = (message: string): TrackerRequest["filter"] => {
	const errors: string[] = [];
	const request = message
		.split(";")
		.map((x) => x.trim())
		.reduce<TrackerRequest["filter"]>((acc, part) => {
	...
	if (errors.length !== 0) {
		throw new Error(`Got ${errors.length} errors while parsing`, {
			cause: errors,
		});
	}
	return request;
